...
async function addElement(type) {
  ...
  if (type === 'raf') {
    ...
    for (let i = 0; i < num; i++) {
      ...
      back.setSize(drawerW, vh * 0.95, t);
      back.updatePos(0, vy, -drawerD / 2 + t / 2);
      back.color = '#795548';
      const left = new CabinetNode('çekmece sol', parent);
      left.setSize(t, vh, drawerD - t * 2);
      left.updatePos(-drawerW / 2 + t / 2, vy, 0);
      left.color = '#795548';
      const right = new CabinetNode('çekmece sağ', parent);
      right.setSize(t, vh, drawerD - t * 2);
      right.updatePos(drawerW / 2 - t / 2, vy, 0);
      right.color = '#795548';
      const front = new CabinetNode('çekmece kapağı', parent);
      front.setSize(drawerW, vh, t);
      front.updatePos(0, vy, drawerD / 2 - t / 2);
      front.color = '#795548';
      const bottom = new CabinetNode('çekmece taban', parent);
      bottom.setSize(drawerW - t * 2, 0.2, drawerD - t * 2);
      bottom.updatePos(0, -cellH / 2 + 0.1, 0);
      bottom.color = '#795548';
      nodes.push(parent);
    }
  } else if (type === 'aski') {
